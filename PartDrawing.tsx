            ...
            {bins.map((bin, binIndex) => {
                ...
                return (
                    <div key={binKey} className="cutting-plan-bin" style={{ marginBottom: '0.5rem' }}>
                         <div className="stock-bar" title={`Visueller Zuschnittplan (gilt für ${bin.count} Stange(n))`} style={{ display: 'flex', height: '30px', background: '#e0e0e0', border: '1px solid #bdbdbd', borderRadius: '4px', overflow: 'hidden', width: '100%', maxWidth: '500px' }}>
                            {cutsAndKerfs.map((item, index) => {
                                const widthPercentage = (item.length / stockLength) * 100;
                                if (item.type === 'kerf') {
                                    return <div key={index} className="kerf-cut" style={{ width: `${widthPercentage}%`, background: 'repeating-linear-gradient(45deg, #aaa, #aaa 2px, #ccc 2px, #ccc 4px)' }} title={`Sägeschnitt: ${(kerf * 1000).toFixed(0)}mm`}></div>;
                                }
                                return (
                                    <div key={index} className="wood-cut" style={{ width: `${widthPercentage}%`, background: '#f0e0c8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', color: '#5d4037', overflow: 'hidden', whiteSpace: 'nowrap', borderRight: '1px solid #bdbdbd' }} title={`Zuschnitt: ${(item.length * 100).toFixed(1)}cm`}>
                                        <span>{(item.length * 100).toFixed(1)}</span>
                                    </div>
                                );
                            })}
                            {waste > 0.01 && (
                                <div className="waste-cut" style={{ flex: '1', background: '#efefef', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', color: '#757575', fontStyle: 'italic', minWidth: '20px' }} title={`Verschnitt: ${(waste * 100).toFixed(1)}cm`}>
                                   <span>~{(waste * 100).toFixed(0)}</span>
                                </div>
                            )}
                        </div>
                    </div>
