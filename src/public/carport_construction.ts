...
import { getDrawingInfo, createBoxDrawingInfo, createMiteredBraceDrawingInfo, createCarportSatteldachRafterDrawing, createCarportPultdachRafterDrawing, optimizeCuttingList } from './drawingUtils.js';

/**
 * Erstellt den JavaScript-Code für das 3D-Modell eines Carports sowie eine exakte Stückliste mit 2D-Zeichnungsinformationen.
 */
export const generateCarportPlan = (params: {
  W: number, D: number, H: number, roofType: RoofType, roofOverhang: number, roofPitch: number,
  POST_DIM: number, BEAM_W: number, BEAM_H: number, TIE_BEAM_H: number, RAFTER_W: number, RAFTER_H: number, BRACE_DIM: number,
  COUNTER_BATTEN_W: number, COUNTER_BATTEN_H: number,
  numberOfPostsPerSide: number,
  middlePurlin: { w: number, h: number } | null
}) => {
    const { W, D, H, roofType, roofPitch, POST_DIM, BEAM_W, BEAM_H, TIE_BEAM_H, RAFTER_W, RAFTER_H, BRACE_DIM, COUNTER_BATTEN_W, COUNTER_BATTEN_H, numberOfPostsPerSide, middlePurlin, roofOverhang } = params;
    ...
    if (roofType === 'Satteldach') {
        ...
        addPart('post', { description: `Pfosten ${(POST_DIM*100).toFixed(1)}x${(POST_DIM*100).toFixed(1)}cm, Länge: ${(postHeight*100).toFixed(1)}cm`, drawingInfo: postDrawing, quantity: numPosts * 2 });
        ...
        addPart('tie_beam', { description: `Zangen/Querhölzer ${(BEAM_W*100).toFixed(1)}x${(BEAM_H*100).toFixed(1)}cm, Länge: ${(tieBeamLength*100).toFixed(1)}cm`, drawingInfo: createBoxDrawingInfo(tieBeamLength, Math.max(BEAM_H, BEAM_W), Math.min(BEAM_H, BEAM_W)), quantity: numPosts});
        ...
        if (kingPostHeight > 0.1) {
            ...
            addPart('king_post', { description: `First-Stütze ${(POST_DIM*100).toFixed(1)}x${(POST_DIM*100).toFixed(1)}cm, Länge: ${(kingPostHeight*100).toFixed(1)}cm`, drawingInfo: kingPostDrawing, quantity: numPosts});
            ...
            if (postZPositions.length > 1) { // Only add braces if there are end posts for them
                addPart(`brace_king_${Math.round(outerLength*100)}`, { description: `Kopfband First-Stütze ${(BRACE_DIM*100).toFixed(1)}x${(BRACE_DIM*100).toFixed(1)}cm, L: ${(outerLength*100).toFixed(1)}cm`, drawingInfo: drawingInfo, quantity: 2});
        ...
        if (middlePurlin && middlePurlin.w > 0 && middlePurlin.h > 0) {
            ...
            if (supportPostHeight > 0.01) {
                addPart('support_post', { description: `Mittelpfetten-Stütze ${(POST_DIM*100).toFixed(1)}x${(POST_DIM*100).toFixed(1)}cm, Länge: ${(supportPostHeight*100).toFixed(1)}cm`, drawingInfo: createBoxDrawingInfo(supportPostHeight, POST_DIM, POST_DIM), quantity: numPosts * 2 });
            }
        }
        ...
        transversalBraceZPositions.forEach(zPos => {
             addPart(`brace_main_trans_${Math.round(mainBraceOuterLength*100)}`, { description: `Kopfband Pfosten (quer) ${(BRACE_DIM*100).toFixed(1)}x${(BRACE_DIM*100).toFixed(1)}cm, L: ${(mainBraceOuterLength*100).toFixed(1)}cm`, drawingInfo: mainBraceDrawing, quantity: 2 });
        ...
        if (postZPositions.length > 1) {
          ...
          addPart(`brace_main_long_${Math.round(mainBraceOuterLength*100)}`, { description: `Kopfband Pfosten (längs) ${(BRACE_DIM*100).toFixed(1)}x${(BRACE_DIM*100).toFixed(1)}cm, L: ${(mainBraceOuterLength*100).toFixed(1)}cm`, drawingInfo: mainBraceDrawing, quantity: 4});
          [-W/2, W/2].forEach(x => {
        ...
        addPart('rafter', { description: `Sparren ${(RAFTER_W*100).toFixed(1)}x${(RAFTER_H*100).toFixed(1)}cm, Länge: ${(rafterLength*100).toFixed(1)}cm`, drawingInfo: rafterDrawing, quantity: numRaftersTotal * 2 });
        ...
        if (middlePurlin && middlePurlin.w > 0 && middlePurlin.h > 0) {
            ...
            const zange_top_y = zange_y + TIE_BEAM_H/2; // Top edge of the cross member
            ...
            if (supportPostHeight > 0.01) {
                addPart('support_post_pult', { description: `Mittelpfetten-Stütze ${(POST_DIM*100).toFixed(1)}x${(POST_DIM*100).toFixed(1)}cm, Länge: ${(supportPostHeight*100).toFixed(1)}cm`, drawingInfo: createBoxDrawingInfo(supportPostHeight, POST_DIM, POST_DIM), quantity: numPosts});
            }
        }
        ...
        addPart('post_high', { description: `Pfosten Hoch ${(POST_DIM*100).toFixed(1)}x${(POST_DIM*100).toFixed(1)}cm, Länge: ${(highPostHeight*100).toFixed(1)}cm`, drawingInfo: getDrawingInfo(highPostPoints, POST_DIM, highPostDims, highPostMarkers), quantity: numPosts});
        ...
        addPart('post_low', { description: `Pfosten Tief ${(POST_DIM*100).toFixed(1)}x${(POST_DIM*100).toFixed(1)}cm, Länge: ${(lowPostHeight*100).toFixed(1)}cm`, drawingInfo: getDrawingInfo(lowPostPoints, POST_DIM, lowPostDims, lowPostMarkers), quantity: numPosts});
        
        // Braces for parts list
        if (numPosts > 1) { // Only add longitudinal braces if there are end posts
            addPart(`brace_high_long_${Math.round(highBraceOuterLength*100)}`, { description: `Kopfband (hoch, längs) ${(BRACE_DIM*100).toFixed(1)}x${(BRACE_DIM*100).toFixed(1)}cm, L: ${(highBraceOuterLength*100).toFixed(1)}cm`, drawingInfo: highBraceDrawing, quantity: 2});
            addPart(`brace_low_long_${Math.round(braceOuterLength*100)}`, { description: `Kopfband (tief, längs) ${(BRACE_DIM*100).toFixed(1)}x${(BRACE_DIM*100).toFixed(1)}cm, L: ${(braceOuterLength*100).toFixed(1)}cm`, drawingInfo: braceDrawing, quantity: 2});
        }
        // Transversal (numPosts on low side)
        addPart(`brace_low_trans_${Math.round(braceOuterLength*100)}`, { description: `Kopfband (tief, quer) ${(BRACE_DIM*100).toFixed(1)}x${(BRACE_DIM*100).toFixed(1)}cm, L: ${(braceOuterLength*100).toFixed(1)}cm`, drawingInfo: braceDrawing, quantity: numPosts});
        ...
        addPart('cross_member', { description: `Zangen ${(BEAM_W*100).toFixed(1)}x${(TIE_BEAM_H*100).toFixed(1)}cm, Länge: ${(crossMemberLength*100).toFixed(1)}cm`, drawingInfo: createBoxDrawingInfo(crossMemberLength, TIE_BEAM_H, BEAM_W), quantity: numPosts});
        ...
        const zange_brace_y_low = zange_y - TIE_BEAM_H / 2 - braceSchenkel / 2;
        ...
        addPart('rafter_sloped', { description: `Sparren ${(RAFTER_W*100).toFixed(1)}x${(RAFTER_H*100).toFixed(1)}cm, Länge: ${(rafterTotalLength*100).toFixed(1)}cm`, drawingInfo: rafterDrawing, quantity: numRaftersTotal });
    ...
    const mainModelCode = `
    const { W, D, H, roofType, roofOverhang, roofPitch, POST_DIM, BEAM_W, BEAM_H, TIE_BEAM_H, RAFTER_W, RAFTER_H, BRACE_DIM, COUNTER_BATTEN_W, COUNTER_BATTEN_H, numberOfPostsPerSide, middlePurlin, braces_3d } = {
